let data = `position=<-30302,  30614> velocity=< 3, -3>
...
function draw() {
    ...
    const avgX = points.reduce((a, p) => a + p.x + p.vx * f, 0) / points.length;
    const avgY = points.reduce((a, p) => a + p.y + p.vy * f, 0) / points.length;

    translate(-avgX, -avgY);

    scale(5);
    for (let p of points) {
        point(p.x + p.vx * f, p.y + p.vy * f, 2, 2);
    }

    const avgDistX = points.reduce((a, p) => a + p.x + p.vx * f - avgX, 0) / points.length;
    const avgDistY = points.reduce((a, p) => a + p.y + p.vy * f - avgY, 0) / points.length;

    // console.log(avgDistX, avgDistY);
}
...
for (let line of lines) {
    let x = parseInt(line.split('position=<')[1].split(',')[0]);
    let y = parseInt(line.split(',')[1].split('>')[0]);

    let vx = parseInt(line.split('velocity=<')[1].split(',')[0]);
    let vy = parseInt(line.split('velocity=<')[1].split(',')[1].split('>')[0]);
