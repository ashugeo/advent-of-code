...
fs.readFile('./data.md', 'utf8', (err, data) => {
    ...
    const ingredientsWithNoAllergens = allIngredients.filter(d => !Object.keys(allergens).includes(d));

    // console.log({ingredientsWithNoAllergens});

    console.log(ingredientsWithNoAllergens.reduce((acc, curr) => {
        acc += foods.filter(d => d.ingredients.includes(curr)).length;
        return acc;
    }, 0));
});